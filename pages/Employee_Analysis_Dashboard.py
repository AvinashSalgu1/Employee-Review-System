...
def main():
    ...
    st.title("Employee Rating Dashboard")
    Employee_df = pd.read_csv("cs_Employee.csv")
    Employee_name = st.selectbox("Employee Name", Employee_df["Name"].tolist(), label_visibility="hidden")
    st.write("")
    st.write("")
    st.write("")
    Employee_id = Employee_df[Employee_df["Name"] == Employee_name]["EmployeeID"].values[0]
    feedback_df = pd.read_csv("cs_feedback.csv")
    feedback_df = feedback_df[feedback_df["EmployeeID"] == Employee_id]

    if not feedback_df.empty:
        emp_col1, col, emp_col2 = st.columns([0.2, 0.8, 0.2])
        avg_Employee_rating = [feedback_df["R1"].mean(), feedback_df["R2"].mean(), feedback_df["R3"].mean(),
                                feedback_df["R4"].mean(), feedback_df["R5"].mean(), feedback_df["R6"].mean(),
                                feedback_df["R7"].mean()]

        with col:
            fig = plot_radar_chart(avg_Employee_rating)
        ...
        with col2:
            ...
            aspects = break_string_into_lines(", ".join([categories[index] for index, value in
                                                         enumerate(avg_Employee_rating) if value < 3]), 38)
